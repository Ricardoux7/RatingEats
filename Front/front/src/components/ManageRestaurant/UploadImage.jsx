...
const UploadImage = ({ restaurantId, imageId, mode = 'add', onUploadSuccess, onClear }) => {
  ...
  return (
    ...
    <div className="bg-white rounded-2xl shadow-md border border-[#258A00] p-6 max-w-md mx-auto flex flex-col items-center gap-4">
      ...
      <h2 className="text-2xl font-bold text-[#258A00] mb-2">
        {mode === 'replace' ? 'Replace Menu Image' : mode === 'post' ? 'Upload New Post Images' : 'Upload New Menu Images'}
      </h2>
      <form onSubmit={handleUpload} className="w-full flex flex-col items-center gap-4">
        <input
          type="file"
          accept="image/jpeg, image/webp"
          multiple={mode !== 'replace' && mode !== 'postUpload'}
        ...
        {selectedFiles.length > 0 && (
          <div className="flex gap-2 flex-wrap">
            ...
            <button
              ...
              onClick={() => {
                setSelectedFiles([]);
                if (mode === 'replace' && onClear) onClear();
              }}
