...
const RestaurantDetails = () => {
  ...
  return (
    <>
      ...
      <div>
        <div className="bg-white overflow-hidden transform transition duration-700  hidden md:block">
          <div className='relative h-[400px] w-full z-0 rounded-2xl flex flex-row items-start'>
            <img src={imageUrl} alt={restaurant.name} className="w-[60%] h-[400px] object-cover rounded-2xl ml-4 relative z-0" />
            <div className='absolute bottom-0 left-0 h-[50%] w-[60%] ml-4 rounded-2xl bg-linear-to-t from-black/70 via-black/30 to-transparent z-10 flex flex-col justify-end p-6'>
              ...
              <div className='flex items-center flex-row '>
                ...
                <p className='text-[1rem] ml-2 text-white'>{restaurant.averageRating}</p>
                <p className='text-[1rem] ml-4 text-white'>{' • ' + restaurant.categories[0]}</p>
              </div>
              <div className='flex items-center flex-row mt-2'>
                <img src="../icons/location2.svg" alt="adress-icon" className="w-5" />
                <span className="ml-2 text-white">{restaurant.adress || "No specified adress"}</span>
              </div>
              <div className="mt-3 flex flex-col items-start space-y-2 h-full">
                <p className="text-white mt-1 w-[70%]">{restaurant.description || "No specified description"}</p>
              </div>
            </div>
          <CreateReservation onSuccess={() => setReservationSuccess(true)}>
          </CreateReservation>
          ...
          <div className='ml-4 mt-4 flex flex-col'>
            ...
            {restaurant.schedule.map((item, i) => (
              <p key={item._id} className='text-[#171A1F] text-[1rem]'>
          ...
          <div className='flex gap-4 p-4 object-cover overflow-x-scroll scrollbar-thin w-[60%] menu-scrollbar'>
              {imagesMenuUrl ? (
                restaurant.menu.map((menuItem, i) => {
                  const menuImageUrl = `${BACKEND_URL}${menuItem.url}`;
                  return (
