/**
 * @file restaurantValidation.middlewares.js
 * @module middlewares/restaurantValidation
 * @description Middlewares de validación para datos de restaurantes usando express-validator.
 */
...
const restaurantValidationRules = [
  ...
  body("geoLocation")
    ...
    .isArray({ min: 1, max: 1 })
    .withMessage(
      'GeoLocation must be an array of one item in "latitude,longitude" format.'
    ),
  ...
  body("schedule")
    ...
    .withMessage("Schedule must be between 3 and 200 characters."),
...
const updateRestaurantValidationRules = [
  ...
  body("phoneNumber")
    ...
    .withMessage(
      "The phone number format is invalid (e.g. 0412xxxxxxx) and must have at least 11 characters."
    ),

  body("capacity")
    .optional()
    .isInt({ min: 1, max: 150 })
    .withMessage("Capacity must be an integer between 1 and 150."),
  ...
  body("geoLocation")
    ...
    .isArray({ min: 2, max: 2 })
    .withMessage(
      "GeoLocation must be an array of two coordinates (latitude, longitude)."
    ),
...
/**
 * Middleware que valida los resultados de express-validator y responde con errores si existen.
 * @function validate
...
const validate = (req, res, next) => {
  ...
  errors
    .array()
    .map((err) => errorsDetails.push({ field: err.param, message: err.msg }));
  return res
    .status(400)
    .json({ message: "Validation failed", errors: errorsDetails });
};
